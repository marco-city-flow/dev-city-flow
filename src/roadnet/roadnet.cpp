...
namespace CityFlow {
    ...
    bool RoadNet::loadFromJson(std::string jsonFileName) {
        ...
        try {
            ...
            for (rapidjson::SizeType i = 0; i < interValues.Size(); i++) {
                ...
                //std::cerr << "intersection readed" << std::endl;
                ...
                for (const auto &lightPhaseValue : lightPhasesValue.GetArray()) {
                    ...
                    //std::cerr << intersections[i].trafficLight.phases.size() << std::endl;
                    //std::cerr << "lightphase pushedback" << std::endl;

                    path.pop_back();
                }
                if (!judgeVirtual)
                {
                    intersections[i].trafficLight.intersection->isVirtual = true;
                }
                //std::cerr << "trafficlights readed" << std::endl;
                path.pop_back(); // End of traffic light
                //std::cerr << "End of traffic light" << std::endl;
                intersections[i].trafficLight.init(0);
                //std::cerr << "traffic light inited" << std::endl;

                path.pop_back(); // End of intersection
                //std::cerr << "End of intersection" << std::endl << std::endl;
            }
            path.pop_back();
            assert(path.empty());
    ...
    void Intersection::initCrosses() {
        ...
        //std::cerr << "alllanelinks size" << n << std::endl;
        for (int i = 0; i < n; i++) {
            for (int j = i + 1; j < n; j++) {
...
FOUND:;
        ...
        //std::cerr << "rubbish goto is done" << n << std::endl;
        ...
        //std::cerr << "crosses pushed back" << n << std::endl;
        ...
        //std::cerr << "lamda strange return done" << n << std::endl;
