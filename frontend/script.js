...
var nodes = {};
var edges = {};
var logs;  // logs for single replay file
var mlogs;  // logs for multiple replay files
var gettingLog = false;

var multipleReplayFiles = false;
...
function handleChooseFiles(v, label_dom) {
    return function (evt) {
        let files = evt.target.files;
        let filenames = "";
        for (let i = 0; i < files.length - 1; i++) { filenames = filenames + files[i].name + ", " };
        filenames += files[files.length - 1].name;
        label_dom.innerText = filenames;
    }
}
...
function uploadFile(v, file, callback) {
    ...
    try {
        ...
        infoAppend("Loading failed: " + file.name);
        console.error(e.message);
    }
}

function uploadFiles(vs, files, callback) {
    try {
        for (let i = 0; i < vs.length; i++) {
            uploadFile(vs[i], files[i], callback);
        }
    } catch (e) {
...
function start() {
    ...
    uploadFile(roadnetData, RoadnetFileDom.files[0], function () {
        if (ReplayFileDom.files.length == 1) {    // uploading one replay file
        ...
        else {   // uploading multiple replay files
            multipleReplayFiles = true;
            uploadFiles(replayData, ReplayFileDom.files, function () {
                let after_update = function () {
                    infoAppend("drawing roadnet");
                    ready = false;
                    document.getElementById("guide").classList.add("d-none");
...
function showCanvas() {
    //nodeCanvas.translate(nodeCanvas.offsetWidth/2, nodeCanvas.offsetHeight/2)
